...
#define UNPACKLONG(OP,T)                        \
   ...
   {                                            \
    ...
    i+=m;                                       \
    doswap(swap,&a,m);                          \
    lua_pushlstring(L,(const char *)&a,m);      \
    ++n;                                        \
    break;                                      \
   }
...
static int l_unpack(lua_State *L)               /** unpack(s,f,[init]) */
{
 ...
 return l_unpack_common(L,s,len,f,i);
}
...
#else
   UNPACKLONG(OP_LONG, long long)
   UNPACKLONG(OP_ULONG, unsigned long long)
#endif
   case ' ': case ',':
    break;
   default:
...
#define PACKLONG(OP,T)                          \
   case OP:                                     \
   {                                            \
    size_t l;                                   \
    const char *d=luaL_checklstring(L,i++,&l);  \
    T a=0;                                      \
    int m=sizeof(l);                            \
    if (m>l)m=l;                                \
    memcpy(&a,d,m);                             \
    doswap(swap,&a,m);                          \
    luaL_addlstring(&b,(void*)&a,m);            \
    break;                                      \
   }
...
#else
   PACKNUMBER(OP_LONG, long long)
   PACKNUMBER(OP_ULONG, unsigned long long)
#endif
   case ' ': case ',':
    break;
   default:
